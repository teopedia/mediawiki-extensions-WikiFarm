...
class SpecialWikiFarm extends SpecialPage {
    ...
    public $admin_wiki;         // admin wiki 
    //public $conf;               // configuration instance
 ...
 * If there is a wiki with '[all]' parameter, then query database for all wikis in it and add to $this->wikis array
 *
 * @param array  $this->wikis 		wiki array
...
//      If WikifarmUpdate = true then update list otherwise use the saved one.
//
...
function wfarmDefineWikis() {
    $out = $this->getOutput();
    $config = $this->getConfig();   // object to get configuration variables from extension.json and LocalSettings.php
    ...
    $this->wikis = $wikis;          // get from WikiFarm_config.php
    //$this->admin_wiki = $wikiAdmin; // get from WikiFarm_config.php
        ...
        if ($wiki->prefix == '[all]') {	  //scan db for all wiki prefixes
            ...
            if ($result = mysqli_query($link, "SHOW TABLES LIKE '%text'")) {    // get all table names with substring "text"
                ...
                while ($row = mysqli_fetch_assoc($result)) {    // Looping through the resultset.
                    ...
                    $this->wikis[] = new wfarmWikiInstance(
                                                    $wiki_url,
                                                    $wiki_api,
                                                    $wiki->db,
                                                    $t_prefix,
...
function wfarmLinks(){
    ...
    //$this->wikis = wfarmDefineWikis( $this->wikis, $wikiAdmin, $infoLevel, $wfarmPrefixDelimiter );	//define wikis to work with
